...
// FIRED BY ACTIONS
function enviarEmailComLaudoAguas() {
     ...
     checkBoxArray.forEach((checkBox,index) => {
      ...
      const linha = index + 4
       ...
       if(checkBox){
         ...
         const nome = aba.getRange(`E${linha}:E${linha}`).getValue()
         const email = aba.getRange(`G${linha}:G${linha}`).getValue()
       
         const linkDaPasta = aba.getRange(`H${linha}:H${linha}`).getValue()
         const empresaVinculo = aba.getRange(`I${linha}:I${linha}`).getValue()
         ...
         const htmlBody = empresaVinculo ?
           `<h3>${empresaVinculo}, <br/>
             ...
             <ul><li><a href="tel:22999911871">22 999911871</a></li>
